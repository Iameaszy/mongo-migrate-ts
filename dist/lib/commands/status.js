...
const status = async (opts) => {
    ...
    try {
        ...
        const notAppliedMigrations = (await migrations_1.loadMigrations(migrationsDir)).filter((migration) => appliedMigrations.find((m) => m.className === migration.className) === undefined);
