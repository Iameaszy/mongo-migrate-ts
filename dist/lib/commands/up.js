...
const up = async (opts) => {
    const { uri, database, options, migrationsCollection, migrationsDir } = config_1.processConfig(opts.config);
    let connection;
    try {
    ...
    try {
        ...
        const migrations = (await migrations_1.loadMigrations(migrationsDir)).filter((migration) => appliedMigrations.find((m) => m.className === migration.className) === undefined);
        if (migrations.length === 0) {
            spinner.warn('No migrations found').stop();
            return;
        }
        for await (const migration of migrations) {
            const localSpinner = ora_1.default(`Applying migration ${migration.className}`).start();
            try {
