...
const downLastAppliedMigration = async (connection, collection) => {
    ...
    const migration = migrationFile.find((m) => m.className === lastApplied.className);
    if (!migration) {
...
const downAll = async (connection, collection) => {
    ...
    const migrationsToUndo = await Promise.all(appliedMigrations.map(async (migration) => {
        const m = await migrations_1.loadMigrationFile(migration.file);
        if (m && m.length === 0) {
            throw new Error(`Can undo migration ${migration.className}, no class found`);
        }
        return m;
    }));
    for await (const migration of flatArray_1.flatArray(migrationsToUndo)) {
        const localSpinner = ora_1.default(`Undoing migration ${migration.className}`).start();
        await migration.instance.down(connection.db);
        await database_1.deleteMigration(collection, migration);
...
const down = async ({ mode, config, }) => {
    const { uri, database, options, migrationsCollection } = config_1.processConfig(config);
