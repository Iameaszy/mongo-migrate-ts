...
const loadMigrationFile = async (filePath) => {
    ...
    const classes = await Promise.resolve().then(() => __importStar(require(path.resolve(filePath))));
...
const loadMigrations = async (migrationsDir) => {
    const fileExt = isTsNode_1.isTsNode() ? new RegExp(/\.ts$/i) : new RegExp(/\.js$/i);
    const migrations = Promise.all(fs
