...
exports.processConfig = exports.getDefaultMigrationsDir = exports.getDefaultConfigPath = exports.getCurrentWorkingDirectory = exports.readConfigFromFile = void 0;
...
const readConfigFromFile = (filePath) => {
    if (!fs.existsSync(filePath)) {
        throw new errors_1.ConfigFileNotFoundError(`Config file ${filePath} not found.`);
    }
...
const getDefaultConfigPath = () => path.join(exports.getCurrentWorkingDirectory(), DEFAULT_CONFIG_FILENAME);
exports.getDefaultConfigPath = getDefaultConfigPath;
const getDefaultMigrationsDir = () => path.join(exports.getCurrentWorkingDirectory(), DEFAULT_MIGRATIONS_DIR);
exports.getDefaultMigrationsDir = getDefaultMigrationsDir;
const getConfigFromEnv = (config) => {
    const uriVarName = (config.environment && config.environment.uriVar) || DEFAULT_ENV_VAR_URI;
    const dbVarName = (config.environment && config.environment.databaseVar) ||
        DEFAULT_ENV_VAR_DB;
...
const processConfig = (config) => {
    ...
    return {
        ...
        migrationsCollection: config.migrationsCollection || DEFAULT_MIGRATIONS_COLLECTION,
